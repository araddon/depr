...
// Implementation of the Bzr/Launchpad interface for managing checkouts
type Bzr struct{}

// Check to see if this dependency is clean or not 
func (s *Bzr) CheckClean(d *Dep) (bool, error) {
	//git diff --exit-code
	cmd := exec.Command("hg", "diff", "--exit-code")
	cmd.Dir = d.AsPath()
	out, err := cmd.Output()
...
func (s *Bzr) Checkout(d *Dep) (bool, error) {
	var cmd *exec.Cmd
	if len(d.As) > 0 {
